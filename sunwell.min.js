...
f.settings.bodyLineHeight;"SPELL"===a.type&&(c.width=460,c.height=290,e.width=460);"WEAPON"===a.type&&(c.width=470,c.height=250,e.width=470);var p=f.settings.bodyFontSize,q=!1;n=a.textMarkdown.replace(/\*\*/g,"").length;75<n&&("SPELL"===a.type&&(q=!0),80<n&&(p=.6*f.settings.bodyFontSize));k.fillStyle="WEAPON"===a.type?"#fff":"#000";k.textBaseline="hanging";k.font=p+'px "'+f.settings.bodyFont+'", sans-serif';y=k.measureText(" ").width;for(z=0;z<u.length;z++){x=u[z];n=x.split("");x=k.measureText(x).width;
if(r+x>e.width||q&&r+x>.6*e.width)q=!1,D=K(h,e,k,r,D,c.width),r=D[0],D=D[1];for(B=0;B<n.length;B++)x=n[B],"*"===x?("*"===n[B+1]&&(m?(m=!1,k.font=" "+p+'px "'+f.settings.bodyFont+'", sans-serif'):(m=!0,k.font="bold "+p+'px "'+f.settings.bodyFont+'", sans-serif')),B+=1):(k.fillText(x,r+f.settings.bodyFontOffset.x,f.settings.bodyFontOffset.y),r+=k.measureText(x).width+y/8);r+=y}K(h,e,k,r,D,c.width);A.push(e);m=G(h);m.h=Math.ceil(m.h/e.height)*e.height;d.drawImage(c,m.x,m.y-2,m.w,m.h,(c.centerLeft-m.w/
